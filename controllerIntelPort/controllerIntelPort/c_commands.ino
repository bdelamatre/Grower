...
/*void readEthernetToBuffer(EthernetClient ethernet, char* commandBuffer, int &bufferPosition, boolean &readyToProcess){
...
void processBuffer(char* commandBuffer){
   ...
   Serial.println(commandBuffer);
    ...
    while (commandBuffer != NULL && thisCommand!=NULL)
    {
     
      
      if(strchr(thisCommand,'?')!=0){
        //Serial.println(strtok(thisCommand,"?"));
        //Serial.println(strtok(NULL,"?"));
        char* parsedCommand = strtok(thisCommand,"?");
        char* parsedQuery = strtok(NULL,"?");
        Serial.println(parsedCommand);
        Serial.println(parsedQuery);
        executeCommand(parsedCommand,parsedQuery);
...
void commandSystemRestart(char* params){
  #if defined(USESERIALMONITOR)
    Serial.println("Restarting system. (BROKEN)");
  #endif
  //restart();
}
